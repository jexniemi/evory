...
export default function Satunnaislukugeneraattori() {
  ...
  const generate = () => {
    setError("");
    if (min >= max) {
      setError("Minimi ei voi olla suurempi kuin maksimi.");
      return;
    }
    const range = max - min + 1;
    if (unique && count > range) {
      setError(`Ainutkertaisia lukuja ei voi olla enemmän kuin ${range}.`);
      return;
    }
    ...
    <div className="flex flex-col gap-5 w-full max-w-sm mx-auto">
      ...
      <label className="flex items-center gap-2 cursor-pointer select-none">
        ...
        <span className="text-sm text-gray-700">
          Ainutkertaiset luvut (ei toistoja)
        </span>
      </label>
