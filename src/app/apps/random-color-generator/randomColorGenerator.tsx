...
export default function RandomColorGenerator() {
  ...
  const newPalette = () => setPalette(Array.from({ length: PALETTE_SIZE }, randomColor))
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6 p-4">
      ...
      >
        <span className={`text-4xl font-black tracking-wider ${isDark(main) ? "text-white" : "text-black"}`}>
          {mainStr}
        </span>
        <span className={`text-sm opacity-70 ${isDark(main) ? "text-white" : "text-black"}`}>
          Click to randomize
        </span>
      </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          <h3 className="font-semibold text-sm text-base-content/70">Random Palette</h3>
          <button className="btn btn-xs btn-outline" onClick={newPalette}>New palette</button>
        </div>
        ...
        <p className="text-xs text-base-content/40 mt-1 text-center">Click a swatch to copy its value</p>
      </div>
    </div>
  )
}
