...
export default function PaycheckCalculator() {
  const calculate = (values: number[]) => {
    ...
    const biweeklyNet = biweeklyGross - federalTax - stateTax - fica - retirementDeduction;
    const monthlyNet = biweeklyNet * 26 / 12;

    return [
      { result: biweeklyGross, label: "Gross pay (biweekly):", suffix: "$", decimals: 2 },
      { result: federalTax, label: "Federal tax:", suffix: "$", decimals: 2 },
      { result: stateTax, label: "State tax:", suffix: "$", decimals: 2 },
      { result: fica, label: "FICA (SS + Medicare):", suffix: "$", decimals: 2 },
      { result: retirementDeduction, label: "401(k) deduction:", suffix: "$", decimals: 2 },
      { result: biweeklyNet, label: "Net pay (biweekly):", suffix: "$", decimals: 2 },
      { result: monthlyNet, label: "Estimated monthly net:", suffix: "$", decimals: 2 },
    ];
  };
