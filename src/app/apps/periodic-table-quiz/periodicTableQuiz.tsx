...
function getChoices(correct: Element, all: Element[], mode: Mode): string[] {
  const wrong = shuffle(all.filter((e) => e.symbol !== correct.symbol)).slice(
    0,
    3,
  );
    ...
    mode === "symbol" ? (
      ...
      <div className="text-center space-y-1">
        ...
        <div className="text-sm text-base-content/50">
          Atomic number: {current.atomicNumber}
        </div>
      </div>
    ) : (
      <div className="text-center space-y-1">
    ...
    <div className="max-w-md mx-auto space-y-4 p-4">
      ...
      {selected && (
        <div className="text-center space-y-2">
          <div
            className={`font-semibold ${state === "correct" ? "text-success" : "text-error"}`}
          >
            {state === "correct"
              ? "✓ Correct!"
              : `✗ It was ${correctAnswer} — ${current.name} (#${current.atomicNumber})`}
          </div>
