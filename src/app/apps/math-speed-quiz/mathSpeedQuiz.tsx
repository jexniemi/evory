...
export default function MathSpeedQuiz() {
  ...
  const goNext = useCallback(
    (wasCorrect: boolean, currentIdx: number, currentScore: number) => {
      stopTimer();
  ...
  if (gameState === "idle" || gameState === "finished") {
    ...
    return (
      <div className="max-w-sm mx-auto space-y-6 p-4">
        ...
        <div>
          <label className="label">
            <span className="label-text font-medium">Operations</span>
          </label>
          <div className="flex gap-2">
            {OPS.map((op) => (
              <button
  ...
  return (
    <div className="max-w-sm mx-auto space-y-4 p-4">
      ...
      <progress
        className={`progress ${timerColor} w-full`}
        value={timeLeft}
      ...
      <div className="grid grid-cols-2 gap-3">
        {choices.map((c) => {
          ...
          if (selected !== null) {
            if (isTimeout) {
              cls = isCorrect
                ? "btn btn-success w-full text-xl font-bold h-16"
                : "btn btn-outline w-full text-xl font-bold h-16 opacity-30";
            } else if (c === selected && isCorrect)
              cls = "btn btn-success w-full text-xl font-bold h-16";
            else if (c === selected && !isCorrect)
              cls = "btn btn-error w-full text-xl font-bold h-16";
            else if (isCorrect)
              cls = "btn btn-success w-full text-xl font-bold h-16 opacity-70";
            else
              cls = "btn btn-outline w-full text-xl font-bold h-16 opacity-30";
          }
          return (
            <button
              key={c}
      ...
      {selected !== null && (
        <div
          className={`text-center font-semibold ${isTimeout ? "text-warning" : selected === q.answer ? "text-success" : "text-error"}`}
        >
          {isTimeout
