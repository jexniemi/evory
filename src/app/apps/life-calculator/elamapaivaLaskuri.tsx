...
export default function ElamapaivaLaskuri() {
  ...
  const totalDays = birth
    ...
    : null;
  const totalHours =
    totalDays !== null ? totalDays * 24 + now.getHours() : null;
  const totalMinutes =
    totalHours !== null ? totalHours * 60 + now.getMinutes() : null;
  const heartbeats =
    totalMinutes !== null ? Math.round(totalMinutes * 70) : null;
  ...
  const totalMonths = birth
    ? Math.floor((now.getTime() - birth.getTime()) / (30.4375 * 86400000))
    : null;
  ...
  const zodiac = birth
    ? getZodiac(birth.getMonth() + 1, birth.getDate())
    : null;
  const birthMonth = birth
    ? `${birth.getDate()}. ${MONTHS_EN[birth.getMonth()]}`
    : null;

  const stats =
    birth && totalDays !== null
      ? [
          {
            icon: "📅",
          ...
          {
            icon: "📆",
            value: totalMonths!.toLocaleString("en-US"),
            label: "Months",
          },
          {
            icon: "⏰",
          ...
          {
            icon: "❤️",
            value: heartbeats!.toLocaleString("en-US"),
            label: "Heartbeats (est.)",
          },
          {
            icon: "💨",
            value: breaths!.toLocaleString("en-US"),
            label: "Breaths (est.)",
          },
          {
            icon: "🎂",
            value: `${nextBirthday} d`,
            label: "Until next birthday",
          },
  ...
  return (
    <div className="flex flex-col gap-5 w-full max-w-lg mx-auto">
      <div>
        ...
        />
        {birthMonth && (
          <p className="text-sm text-gray-400 mt-1">Born {birthMonth}</p>
        )}
      </div>
