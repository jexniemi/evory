...
export default function ColorContrastChecker() {
  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-md mx-auto">
      ...
      <button className="btn btn-outline btn-sm self-center" onClick={swap}>
        ⇄ Swap Colors
      </button>
      ...
      {result && (
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div
              className={`card p-4 text-center ${result.aaNormalText ? "bg-success/20" : "bg-error/20"}`}
            >
              <div className="text-xl font-bold">
                {result.aaNormalText ? "Pass ✓" : "Fail ✗"}
              </div>
              <div className="text-xs opacity-70">AA Normal Text</div>
              <div className="text-xs opacity-50">≥ 4.5:1</div>
            </div>
            <div
              className={`card p-4 text-center ${result.aaLargeText ? "bg-success/20" : "bg-error/20"}`}
            >
              <div className="text-xl font-bold">
                {result.aaLargeText ? "Pass ✓" : "Fail ✗"}
              </div>
              <div className="text-xs opacity-70">AA Large Text</div>
              <div className="text-xs opacity-50">≥ 3:1</div>
            </div>
            <div
              className={`card p-4 text-center ${result.aaaNormalText ? "bg-success/20" : "bg-error/20"}`}
            >
              <div className="text-xl font-bold">
                {result.aaaNormalText ? "Pass ✓" : "Fail ✗"}
              </div>
              <div className="text-xs opacity-70">AAA Normal Text</div>
              <div className="text-xs opacity-50">≥ 7:1</div>
            </div>
            <div
              className={`card p-4 text-center ${result.aaaLargeText ? "bg-success/20" : "bg-error/20"}`}
            >
              <div className="text-xl font-bold">
                {result.aaaLargeText ? "Pass ✓" : "Fail ✗"}
              </div>
