...
const calculate = (values: number[]) => {
  ...
  if (hip <= 0)
    return [
      {
        result: 0,
        label: "Hip circumference must be greater than 0",
        suffix: "",
      },
    ];
  ...
  if (isMale) {
    if (ratio < 0.9) {
      riskScore = 1;
      riskLabel = "Low health risk (Male: WHR < 0.90)";
    } else if (ratio < 1.0) {
      riskScore = 2;
      riskLabel = "Moderate health risk (Male: WHR 0.90–0.99)";
    } else {
      riskScore = 3;
      riskLabel = "High health risk (Male: WHR ≥ 1.00)";
    }
  } else {
    if (ratio < 0.8) {
      riskScore = 1;
      riskLabel = "Low health risk (Female: WHR < 0.80)";
    } else if (ratio < 0.85) {
      riskScore = 2;
      riskLabel = "Moderate health risk (Female: WHR 0.80–0.84)";
    } else {
      riskScore = 3;
      riskLabel = "High health risk (Female: WHR ≥ 0.85)";
    }
  }
