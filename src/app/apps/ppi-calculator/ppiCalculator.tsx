  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="form-control">
          <label className="label">
            <span className="label-text text-sm">Screen Size (inches)</span>
          </label>
          <input
            type="number"
            className="input input-bordered input-sm"
      ...
      {result && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-center">
            ...
            <Stat
              label="Aspect Ratio"
              value={`${result.aspectW}:${result.aspectH}`}
            />
            <Stat
              label="Display Size"
              value={`${result.widthCm} × ${result.heightCm} cm`}
            />
            <Stat
              label="Total Pixels"
          ...
          <div className="text-sm mt-1">
            {Number(result.ppi) >= 300 ? (
              <div className="badge badge-success gap-1">
                Retina-grade (300+ PPI)
              </div>
            ) : Number(result.ppi) >= 200 ? (
              <div className="badge badge-info gap-1">
                High density (200+ PPI)
              </div>
...
function Stat({
  ...
  return (
    <div
      className={`stat rounded-lg p-3 ${highlight ? "bg-primary text-primary-content" : "bg-base-200"}`}
    >
      <div
        className={`stat-title text-xs ${highlight ? "text-primary-content/70" : ""}`}
      >
        {label}
      </div>
