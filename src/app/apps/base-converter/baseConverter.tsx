...
const BASES: { base: Base; label: string; prefix: string; pattern: RegExp }[] = [
  ...
  { base: 16, label: "Hexadecimal (Base 16)", prefix: "0x", pattern: /^[0-9a-fA-F]+$/ },
]
  ...
  return (
    <div className="max-w-lg mx-auto space-y-3 p-4">
      {BASES.map(({ base, label, prefix }) => (
        <div key={base} className="card bg-base-100 border border-base-300 shadow-sm">
          <div className="card-body p-4">
            <div className="flex items-center justify-between mb-2">
              <label className="label-text font-semibold text-sm">{label}</label>
              <button
            ...
            <div className="flex items-center gap-2">
              {prefix && (
                <span className="text-sm font-mono text-base-content/40 shrink-0">{prefix}</span>
              )}
              <input
                type="text"
